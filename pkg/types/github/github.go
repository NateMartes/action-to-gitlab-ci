...
//
//    githubWorkflow, err := UnmarshalGithubWorkflow(bytes)
//    bytes, err = githubWorkflow.Marshal()
...
func UnmarshalGithubWorkflow(data []byte) (GithubWorkflow, error) {
	var r GithubWorkflow
	err := json.Unmarshal(data, &r)
	return r, err
}

func (r *GithubWorkflow) Marshal() ([]byte, error) {
...
type OnClass struct {
	...
	PullRequest                                                                                                                                                     *PullRequestClass         `json:"pull_request"`
	...
	PullRequestTarget                                                                                                                                               *PullRequestTargetClass   `json:"pull_request_target"`
	...
	// You can optionally specify a timezone using an IANA timezone string                                                                                                                    
	// (https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) for timezone-aware                                                                                                      
	// scheduling. Scheduled workflows run on the latest commit on the default or base branch.                                                                                                
...
type PullRequestTargetClass struct {
	Branches       []string `json:"branches,omitempty"`
	BranchesIgnore []string `json:"branches-ignore,omitempty"`
	Paths          []string `json:"paths,omitempty"`
	PathsIgnore    []string `json:"paths-ignore,omitempty"`
...
type Schedule struct {
	// A cron expression that represents a schedule. A scheduled workflow will run at most once        
	// every 5 minutes.                                                                                
	Cron                                                                                       string  `json:"cron"`
	// A string that represents the time zone a scheduled workflow will run relative to in IANA        
	// format (e.g. 'America/New_York' or 'Europe/London'). If omitted, the workflow will run          
...
type PermissionsEvent struct {
	...
	ArtifactMetadata   *PermissionsLevel `json:"artifact-metadata,omitempty"`
...
func (x *Types) MarshalJSON() ([]byte, error) {
